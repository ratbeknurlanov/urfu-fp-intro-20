...
dequeue (Queue left (x:xr)) = (x, Queue left xr)
...
instance IntArray [Int] where
  ...
  count l =
    let
    c = take (succ (maximum l)) (repeat 0)
    run acc xs = case xs of
      [] -> acc
      x:xs -> run (replace acc x (succ (acc !! x))) xs
    in
      run c l
  expand [] = []
  expand l = concat [take (l !! i) (repeat i) | i <- [0..length l - 1]]

instance IntArray (Array Int Int) where
  count arr = accumArray (+) 0 (minimum arr, maximum arr) (zip (arr) [1,1..])
  expand arr =
    let
      sorted = concat [take (snd assoc) (repeat (fst assoc)) | assoc <- assocs arr]
    in
      array (minimum sorted, maximum sorted) [(i, sorted !! i) | i <- [0..length sorted - 1]]

instance IntArray (Map.IntMap Int) where
  count m = Map.fromListWith (+) (zip m [1,1..])
  expand m =
    let
      sorted = concat $ Map.elems $ Map.mapWithKey (\k v -> take v (repeat k)) m
    in
      Map.fromList [(i, sorted !! i) | i <- [0..length sorted - 1]]

-- Сортирует массив целых неотрицательных чисел по возрастанию
countingSort :: forall a. IntArray a => [Int] -> [Int]
countingSort l = expand $ count l
