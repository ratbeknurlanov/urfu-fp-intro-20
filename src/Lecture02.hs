...
foldl _ initial Nil = initial
-- foldl f initial (head :. tail) = (foldl f initial tail) f head
...
-}
concat :: [[a]] -> [a]
concat l = P.foldr (\x acc -> x ++ acc) [] l
...
-}
intercalate :: [a] -> [[a]] -> [a]
intercalate sep l = P.foldr (\x acc -> x ++ sep ++ acc) (last l) (init l)
